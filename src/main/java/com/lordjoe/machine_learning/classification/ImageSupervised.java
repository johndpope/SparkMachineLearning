  ...
  //      labeledPoints = labeledPoints.map(new GaborWaveletFeature(MINST_IMAGE_WIDTH,MINST_IMAGE_HEIGHT) );
   //     labeledPoints = labeledPoints.map(new HoughTransform(MINST_IMAGE_WIDTH,MINST_IMAGE_HEIGHT) );
        ...
        LogisticRegressionWithLBFGS logisticRegressionWithLBFGS = new LogisticRegressionWithLBFGS()
                .setNumClasses(10);
        final LogisticRegressionModel model = logisticRegressionWithLBFGS.run(training.rdd());
    ...
    public static void main(String[] args) {
        ...
        Logger.getLogger("LBFGS").setLevel(Level.WARN);
