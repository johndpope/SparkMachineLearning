...
public class HoughLineTransform implements Function<LabeledPoint, LabeledPoint>, Serializable {
    ...
    public BufferedImage getHoughArrayImage() {
        ...
        for (int t = 0; t < maxTheta; t++) {
            for (int r = 0; r < doubleHeight; r++) {
                ...
                int v = 255 - (int) value;
                int c = new java.awt.Color(v, v, v).getRGB();
                image.setRGB(t, r, c);
            }
        }
        return image;
    ...
    public  LabeledPoint  call(LabeledPoint v1) throws Exception {
        ...
        this.initialise();
        org.apache.spark.mllib.linalg.Vector features = v1.features();
        int index = 0;
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                double apply = features.apply(index++);
                if (apply > 0)
                    addPoint(i, j);
            }
        }
        index = 0;
        List<HoughLine> lines = getLines(THRESHOLD);
        double[] fvalues = new double[2 * MAX_LINES];
        if(lines != null ) {
            if (lines.size() > MAX_LINES)
                lines = lines.subList(0, MAX_LINES);
            for (HoughLine line : lines) {
                fvalues[index++] = line.r;
                fvalues[index++] = line.theta;
            }
        }
        Vector fvector = Vectors.dense(fvalues);
        LabeledPoint ret1 = LabeledPoint.apply(v1.label(), fvector);
       return ret1;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        HoughLineTransform h = new HoughLineTransform(image.getWidth(), image.getHeight());
