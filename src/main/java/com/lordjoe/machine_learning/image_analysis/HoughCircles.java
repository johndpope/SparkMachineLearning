...
/**
 * com.lordjoe.machine_learning.image_analysis.HoughCircles
 * User: Steve
 * Date: 3/22/2016
 * houghCircles_.java:
 * <p>
 ...
 *   must be in black).
 */
public class HoughCircles implements Function<LabeledPoint, LabeledPoint>, Serializable {

    public static final int DEFAULT_MIN_RADIUS = 10;
    public static final int DEFAULT_MAX_RADIUS = 20;
    public static final int DEFAULT_INCREMENT_RADIUS = 2;
    public static final int DEFAULT_NUMBER_CIRCLES = 10;
    public static final int DEFAULT_THRESHOLD = 60;


    public int radiusMin = DEFAULT_MIN_RADIUS;  // Find circles with radius grater or equal radiusMin
    public int radiusMax = DEFAULT_MAX_RADIUS;  // Find circles with radius less or equal radiusMax
    public int radiusInc = DEFAULT_INCREMENT_RADIUS;  // Increment used to go from radiusMin to radiusMax
    public int maxCircles = DEFAULT_NUMBER_CIRCLES; // Numbers of circles to be found
    ...
    public final int width; // Hough Space width (depends on image width)
    public final int height;  // Hough Space heigh (depends on image height)
    ...
    public HoughCircles(int width, int height) {
        this.width = width;
        this.height = height;
...
//    }
    ...
    public LabeledPoint call(LabeledPoint labeledPoint) throws Exception {
        java.util.List<LabeledPoint> ret = new ArrayList<>();
        org.apache.spark.mllib.linalg.Vector features = labeledPoint.features();
        imageValues = new int[features.size()];
        for (int i = 0; i < features.size(); i++) {
            double apply = features.apply(i);
            if (apply != 0)
                imageValues[i] = (int) apply;
        }
        ...
        double[] data = extractData(maxCircles);


        return new LabeledPoint(labeledPoint.label(), Vectors.dense(data));
    }

    private double[] extractData(int maxCircles) {
        double[] ret = new double[3 * maxCircles];
        int xMax = 0;
        int yMax = 0;
        int rMax = 0;
        int index = 0;
