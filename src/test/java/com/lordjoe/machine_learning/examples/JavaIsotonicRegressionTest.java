...
public class JavaIsotonicRegressionTest {
    @Test
    public void testIsotonicRegression() {
        SparkUtilities.setAppName("JavaIsotonicRegressionExample");
        ...
        JavaPairRDD<Double, Double> predictionAndLabel = test.mapToPair(
                new Tuple3DoubleDoublePairFunction(model)
        );
        ...
        // Save and load model
        String path = "target/tmp/myIsotonicRegressionModel";

        SparkUtilities.saveModel(model, path);
        IsotonicRegressionModel sameModel = IsotonicRegressionModel.load(jsc.sc(), path);
        // $example off$
    }

    private static class Tuple3DoubleDoublePairFunction implements PairFunction<Tuple3<Double, Double, Double>, Double, Double>, Serializable {
        private final IsotonicRegressionModel m_model;

        public Tuple3DoubleDoublePairFunction(IsotonicRegressionModel model) {
            m_model = model;
        }

        @Override
        public Tuple2<Double, Double> call(Tuple3<Double, Double, Double> point) {
            Double predictedLabel = m_model.predict(point._2());
            return new Tuple2<Double, Double>(predictedLabel, point._1());
        }
    }

    private static class StringTuple3Function implements Function<String, Tuple3<Double, Double, Double>>, Serializable {
